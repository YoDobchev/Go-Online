        ...
        (async () => {
            try {
                ...
                if (!res.ok) {
                    ...
                    throw new Error(
                        text || `Failed to load reports (${res.status})`,
                    );
                }
    ...
    const handleDelete = async (reportId: number) => {
        try {
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error(
                    text || `Failed to delete report (${res.status})`,
                );
            }
                ...
                {!loading && !error && reports.length > 0 && (
                    <div className="reports-table">
                        <div className="reports-table__header reports-table__grid">
                            ...
                            <div>From</div>
                            <div>Created</div>
                            <div className="reports-table__actionsHeader">
                                Resolved
                            </div>
                        </div>
                        ...
                        {reports.map((r) => (
                            <div
                                key={r.id}
                                className="reports-table__row reports-table__grid"
                            >
